# Responsive Web Design Certification 🎨 [freeCodeCamp]

Questo repository documenta il mio percorso di specializzazione nel design dell'interfaccia utente e nello sviluppo front-end attraverso le sfide e i progetti di freeCodeCamp.

L'obiettivo è consolidare le competenze tecniche e metodologiche acquisite durante il mio percorso formativo, preparandomi alla gestione di logiche applicative più complesse in JavaScript e all'integrazione di soluzioni basate su AI.

---

### 👤 Profilo & Links

- **Libanio Leoncini** – Junior Full Stack Developer
- **Portfolio:** [portfolio-psi-lilac-74.vercel.app](https://portfolio-psi-lilac-74.vercel.app/)
- **Contatti:** [LinkedIn](https://www.linkedin.com/in/libanio-leoncini/) | [GitHub](https://github.com/LLeo93)

### 🎓 Formazione & Certificazioni

Il mio approccio allo sviluppo web si basa su un percorso formativo continuo e multidisciplinare:

- **Master Full Stack Developer:** Percorso intensivo di 6 mesi presso EPICODE focalizzato su metodologie Agile/Scrum e architetture software complete.
- **Specializzazione Front-End:** Focus su accessibilità web (A11y), ottimizzazione SEO e UX design presso Synergie Academy.
- **Certificazione DigComp 2.2 (Regione Lombardia):** Validazione ufficiale delle competenze digitali in sicurezza, protezione dati, creazione di contenuti e collaborazione digitale.
- **AI & Future Tech:** Appassionato di intelligenza artificiale, ne studio l'integrazione nel ciclo di sviluppo per ottimizzare il problem-solving e la qualità del codice.

### 🛠️ Core Stack in questa Certification

In questo specifico modulo sto approfondendo:

- **Responsive Architecture:** Gestione avanzata di unità relative (`rem`, `em`, `vw`, `vh`) e approccio _Mobile-First_.
- **Modern Layouts:** Utilizzo intensivo di Flexbox e CSS Grid per la creazione di interfacce fluide.
- **Web Accessibility (A11y):** Implementazione di standard inclusivi per garantire la migliore UX a ogni utente.
- **Visual Styling:** Trasformazioni, filtri, animazioni e manipolazione avanzata del box model.

### 📈 Roadmap

- [x] HTML5 & CSS Fundamentals
- [ ] Responsive Web Design Projects (In corso)
- [ ] JavaScript Algorithms & Data Structures (Next Step)

---

_“Dalla passione per la modifica di codice alla creazione di soluzioni software complete.”_
